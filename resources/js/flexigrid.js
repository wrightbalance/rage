	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			onTimeout: function () {
				...
				$('.pPageStat',this.pDiv).html(p.timeoutMsg);	
				this.loading = false;
				if (p.onTimeout)
					{
					p.onTimeout();
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					$.ajax({
					   ...
					   complete: function (data,textStatus) { if (textStatus == 'timeout') g.onTimeout(); },
		...
		$('.pcontrol input',g.pDiv).keydown(function(e){if(e.keyCode==13) {e.preventDefault(); g.changePage('input');} });
