<?
    ...
    foreach ($db as $row)
        ...
        $rows[] = array(
                "id" => $row['_id'],
                "cell" => array(
                	 "<a href=\"/cms/page/edit/".$row['_id']."\" class=\"view\" data-aid=\"".$row['_id']."\">".$row['page_title']."</a>"
                	,date('M d, Y',strtotime($row['created']))
                	,$row['author']
                	,"<a href=\"/cms/page/edit/".$row['_id']."\" class=\"viewpage\" data-aid=\"".$row['_id']."\"><i class=\"icon-pencil\"></i>  Edit</a>"
